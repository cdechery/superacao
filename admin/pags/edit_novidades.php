...
if( !isset($_SESSION["Usuario_Logado"]) || $_SESSION["Usuario_Logado"] != "Admin" ){header("location: login.php");exit;}
  ...
  if(isset($_POST['submit'])){
    ...
    $nimg = arquivosimg("SDupload","novidades",$id_alt,"miniatura");
    $editor1format = $_POST["editor1"];
    update("novidades","novidades",$id_alt);
...
pagEdicao('Novidades','novidades','Editar');
?>
        ...
        <div class="conteudo">
            ...
            <?php
            $id_alt                = isset($_GET['id_alt']) ? $_GET['id_alt'] : '';
            $EditArt               = mysql_query('SELECT * FROM novidades WHERE ID="'.$id_alt.'"') or die(ErroBanco(59));
            ...
            echo '<form method="post" action="index.php?id=edit_novidades&amp;acao='.$acaodepag.'&amp;id_alt='.$id_alt.'" enctype="multipart/form-data">';
