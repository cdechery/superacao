...
if( !isset($_SESSION["Usuario_Logado"]) || $_SESSION["Usuario_Logado"] != "Admin" ){header("location: login.php");exit;}
  ...
  if(isset($_POST['submit'])){
    $ID_Cidade_C = isset($_GET['id_alt']) ? $_GET['id_alt'] : '';
     $NomedoArquivo = arquivosimg("","inifotos","","miniatura");
     cadastrar("inifotos","");
    
        echo '<script type="text/javascript">';
        echo 'alert(\'Dados registrados com sucesso!\');';
        echo '</script>';
        echo '<script>window.location="'.$_SERVER["PHP_SELF"].'?id=inifotos&acao=iniciativa&id_alt='.$ID_Cidade_C.'";</script>';
        unset($_FILES);
        unset($_POST);
    
}
pagEdicao('Iniciativa','iniciativa','Cadastrar fotos de iniciativa');
?>
        ...
        <div class="conteudo">
            ...
            echo '<form method="post" action="index.php?id=inifotos&amp;acao=iniciativa&amp;id_alt='.$pegaID.'" enctype="multipart/form-data">';
            ...
            echo '</form>';
            echo '<hr />';
                  listas("inifotos","","",$pegaID,"inifotos");
            ?>
