...
if( !isset($_SESSION["Usuario_Logado"]) || $_SESSION["Usuario_Logado"] != "Admin" ){header("location: login.php");exit;}
  ...
  if(isset($_POST['submit'])){
    $ID_Cidade_C = isset($_GET['id_alt']) ? $_GET['id_alt'] : '';
     $NomedoArquivo = arquivosimg("","novfotos","","miniatura");
     cadastrar("novfotos","");
    
        echo '<script type="text/javascript">';
        echo 'alert(\'Dados registrados com sucesso!\');';
        echo '</script>';
        echo '<script>window.location="'.$_SERVER["PHP_SELF"].'?id=novfotos&acao=novidades&id_alt='.$ID_Cidade_C.'";</script>';
        unset($_FILES);
        unset($_POST);
    
}
pagEdicao('Novidades','novidades','Cadastrar fotos de novidades');
?>
        ...
        <div class="conteudo">
            ...
            echo '<form method="post" action="index.php?id=novfotos&amp;acao=novidades&amp;id_alt='.$pegaID.'" enctype="multipart/form-data">';
            ...
            echo '</form>';
            echo '<hr />';
                  listas("novfotos","","",$pegaID,"novfotos");
            ?>
